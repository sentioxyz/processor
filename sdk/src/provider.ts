...
export function getProvider(networkish?: Networkish): Provider {
  ...
  if (!global.PROCESSOR_STATE.SentioProvider) {
    throw Error('Provider not found')
  }
  const value = global.PROCESSOR_STATE.SentioProvider.get(network.chainId)
  if (value === undefined) {
    throw Error('Provider not found')
  }
  return value
...
export function setProvider(config: any, concurrency = 4) {
  global.PROCESSOR_STATE.SentioProvider = new Map<number, Provider>()
  ...
  for (const chainIdStr in config) {
    ...
    global.PROCESSOR_STATE.SentioProvider.set(chainId, provider)
  }
}
